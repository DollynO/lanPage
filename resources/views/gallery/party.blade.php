...
<x-layouts.app>
  ...
  <div class="flex h-full w-full flex-1 flex-col gap-4 rounded-xl">
        <div class="py-12 bg-transparent z-0">
          <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 z-0">
                <h1 class="flux">
                    {{ $party->name }}  {{ __('Gallery') }}
                </h1>
            @auth
                @if(auth()->user()->can_upload)
                    <button id="addPhotosBtn"
                            class="fi-color fi-color-primary fi-bg-color-400 hover:fi-bg-color-300 dark:fi-bg-color-600 dark:hover:fi-bg-color-500 fi-text-color-800 hover:fi-text-color-800 dark:fi-text-color-950 dark:hover:fi-text-color-900 fi-btn fi-size-md  fi-ac-btn-action neon-button-primary">
                        + Add Photos
                    </button>
                @endif
            @endauth
    ...
    <div id="uploadModal"
         class="fixed inset-0  bg-black/50 hidden z-50 overflow-auto px-4 py-8">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            closeBtn.addEventListener('click', () => {
                ...
                if (!previews.length || !uploadBtn.disabled) { // prevent closing mid-upload
