                            ...
                            <tr class="bg-white border-b hover:bg-gray-50">
                                ...
                                <x-td>
                                    ...
                                    <button wire:click="increaseVotes({{ $suggestion->id }})"
                                            class="outline-none inline-flex justify-center items-center group transition-all ease-in duration-150 focus:ring-2 focus:ring-offset-2 hover:shadow-sm disabled:opacity-80 disabled:cursor-not-allowed rounded gap-x-2 text-sm w-10 py-2 ring-blue-500 text-white bg-blue-500 hover:bg-blue-600 hover:ring-blue-600 positive"
                                            @if ($suggestion->userVote(Auth::id()) || $suggestionsLeft == 0) style="display: none" @endif>
                                        +1
                                    </button>
                                    <button wire:click="decreaseVotes({{ $suggestion }})"
                                            class="outline-none inline-flex justify-center items-center group transition-all ease-in duration-150 focus:ring-2 focus:ring-offset-2 hover:shadow-sm disabled:opacity-80 disabled:cursor-not-allowed rounded gap-x-2 text-sm w-10 py-2 ring-red-500 text-white bg-red-500 hover:bg-red-600 hover:ring-red-600 negative"
                                    ...
                                    <button wire:click="removeSuggestion({{ $suggestion->id }})"
                                            class="outline-none inline-flex justify-center items-center group transition-all ease-in duration-150 focus:ring-2 focus:ring-offset-2 hover:shadow-sm disabled:opacity-80 disabled:cursor-not-allowed rounded gap-x-2 text-sm w-10 py-2 ring-red-500 text-white bg-red-500 hover:bg-red-600 hover:ring-red-600 negative trash"
                                            @if (!$suggestion->userVote(Auth::id()) || $suggestion->votes()->count() > 1) style="display: none" @endif>
